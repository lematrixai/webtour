...
const TourDetail = () => {
  ...
  // Calculate video width based on scroll
  const videoWidth = Math.max(0, 100 - (scrollY / 10)); // Shrink from 100% to 0% as you scroll

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#18130C] via-[#18130C] to-[#18130C]/90">
      {/* Fullscreen Hero Video - Shrinks on scroll */}
      <div 
        className="fixed top-0 left-0 h-screen z-0 transition-all duration-300 ease-out"
        style={{ width: `${videoWidth}%` }}
      >
        <video
          className="w-full h-full object-cover"
          src={destinationData.videos[0].url}
          autoPlay
          muted
          loop
          playsInline
        />
        {/* Overlay - only show when video is large enough */}
        {videoWidth > 30 && (
          <div className="absolute inset-0 bg-black/40 z-10 flex flex-col items-center justify-center">
            <h1 className="text-4xl md:text-6xl font-bold text-white drop-shadow-lg mb-6 text-center">
              {destinationData.title}
            </h1>
            <button
              onClick={handleScrollDown}
              className="mt-8 animate-bounce bg-[#E1C5A0]/80 hover:bg-[#E1C5A0] text-[#18130C] rounded-full p-3 shadow-lg z-20"
      ...
      {/* Main Content - Slides up over the shrinking video */}
      <div ref={mainContentRef} className="relative z-10 pt-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Main Content */}
            <div className="lg:col-span-2 space-y-8">
              {/* Video Gallery (hidden on top, keep for thumbnails if needed) */}
              <div className="space-y-4">
                <div className="aspect-[16/9] rounded-2xl overflow-hidden relative">
                  ...
                  >
                    <source
                      src="https://res.cloudinary.com/dj7odelpw/video/upload/v1/sw6efwh8g7ddydtjksfo"
                      type="video/mp4"
                    />
                  </video>
